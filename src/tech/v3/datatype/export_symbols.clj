            ...
            (mapv
             (fn [sym-name]
               (let [varval (requiring-resolve (symbol (name src-ns)
                     ...
                     full-meta (-> (select-keys var-meta
                                                [:file :line :column
                                                 :doc
                                                 :tag
                                                 :arglists])
                                   (assoc :export-info (list 'quote {:src-ns src-ns
                                                                     :src-sym sym-name})))
                     full-meta (if (contains? full-meta :arglists)
                                 (update full-meta :arglists (fn [v] (list 'quote v)))
                                 full-meta)
                     full-meta (if (contains? full-meta :tag)
                                 (update full-meta :tag (fn [v] (list 'quote v)))
                                 full-meta)]
                 (when-not varval
                   (throw (ex-info
