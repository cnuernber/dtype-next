				      ...
				      Indexed, PrimitiveReader
{
  ...
  default byte readByte(long idx) {return RT.uncheckedByteCast(read(idx));}
  default short readShort(long idx) {return RT.uncheckedShortCast(read(idx));}
  default int readInt(long idx) {return RT.uncheckedIntCast(read(idx));}
  default long readLong(long idx) {return RT.uncheckedLongCast(read(idx));}
  default float readFloat(long idx) {return RT.uncheckedFloatCast(read(idx));}
  default double readDouble(long idx) {return read(idx);}
  default Object getElemwiseDatatype () { return Keyword.intern(null, "float64"); }
